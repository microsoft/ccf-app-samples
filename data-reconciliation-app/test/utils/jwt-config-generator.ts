...
export class JwtConfigsGenerator {
  ...
  public static async getDigiCertGlobalRootCA(): Promise<string> {
    /** returned the certificate as string instead of download it,
     *  because downloading the certificate pem file cause a timeout issue in the pipeline  
     */
    //const ca_cert = await axios({ url: "https://cacerts.digicert.com/DigiCertGlobalRootCA.crt", method: 'GET', responseType: 'blob' });
    //return ca_cert.data;
